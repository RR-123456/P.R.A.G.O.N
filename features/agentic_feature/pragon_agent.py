...
def get_base_dir():
    ...
    # This file lives at project/features/agentic_feature/pragon_agent.py,
    # so we need to go up three levels to reach the project root (same
    # root pragon_main.py itself resolves to).
...
Code for {file_path}:"""
    ...
    try:
        ...
        print(f"[Pragon FileForger] ✅ Written: {file_path} ({len(code)} chars)")
        return code
...
def _install_dependencies(dependencies: list[str], project_dir: Path) -> str:
    ...
    for dep in dependencies:
        ...
        if result.returncode != 0:
            ...
            print(f"[Pragon FileForger] ✓ Already installed: {pkg_name}")

    if not to_install:
        return f"All dependencies already installed: {', '.join(dependencies)}"

    print(f"[Pragon FileForger] 📦 Installing: {to_install}")
    try:
        result = subprocess.run(
...
def _open_vscode(project_dir: Path) -> bool:
    ...
    for cmd in vscode_candidates:
        try:
            ...
            time.sleep(1.5)
            print(f"[Pragon FileForger] 💻 VSCode opened: {project_dir}")
            return True
        except Exception:
            continue
    return False
...
def _run_project(run_command: str, project_dir: Path, timeout: int = 30) -> str:
    print(f"[Pragon FileForger] 🚀 Running: {run_command}")
    try:
...
def _try_auto_install(error_output: str, project_dir: Path) -> bool:
    ...
    print(f"[Pragon FileForger] 🔧 Auto-installing missing package: {pkg}")
    try:
        result = subprocess.run(
...
Fixed code for {fix_path}:"""
        ...
        try:
            ...
            print(f"[Pragon FileForger] 🔧 Fixed: {fix_path}")

        except Exception as e:
            if _is_rate_limit(e):
                raise RateLimitError(str(e))
            print(f"[Pragon FileForger] ⚠️ Could not fix {fix_path}: {e}")
...
def _build_project(
    ...
    def log(msg: str):
        print(f"[Pragon FileForger] {msg}")
        if player:
            player.write_log(f"[Pragon FileForger] {msg}")
...
def pragon_agent_action(
    ...
    """
    Entry point wired to the "pragon_agent" tool in pragon_main.py.
    Accepts either "goal" (the tool-call schema used across the rest of
    the app) or "description" (this module's original param name) so it
    works as a drop-in replacement for the old dev_agent.
    """
    p            = parameters or {}
    description  = (p.get("goal") or p.get("description") or "").strip()
    language     = (p.get("language") or "python").strip()
    ...
    if session_memory:
        try:
            session_memory.set_last_search(query=f"pragon_agent:{description[:60]}", response=result[:200])
        except Exception:
            pass
...
pragon_fileforger = pragon_agent_action