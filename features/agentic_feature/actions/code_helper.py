# features/agentic_feature/actions/code_helper.py
#
# AI-powered code assistant for the planner/executor agent — writes,
# edits, explains, runs, builds, debugs, and optimizes a SINGLE file.
# (Distinct from features/feature/code_helper.py's code_helper_action,
# which is a lighter inline review/debug/generate tool that never
# touches disk — this one saves real files, same as pragon_fileforger
# does for whole projects.)
#
# Actions:
...
#   screen_debug → Screenshot the screen, analyze/fix visible error with Gemini
...
def get_base_dir():
    ...
    # This file lives at project/features/agentic_feature/actions/code_helper.py,
    # so go up four levels to reach the project root.
...
def code_helper(
    ...
    """
    Called from the agent executor (and can be called standalone too).
