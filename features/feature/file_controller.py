...
def _is_safe_path(target: Path) -> bool:
    """Checks if the given path is within _SAFE_ROOTS. Rejects the operation otherwise."""
    try:
...
def move_file(path: str, name: str = "", destination: str = "") -> str:
    try:
        ...
        try:
            ...
            final_dst = dst  # captured for the closure below
            push_undo(
                f"moved {final_dst.name} to {final_dst.parent.name}/",
                lambda: shutil.move(str(final_dst), orig_src),
            )
        except Exception:
            pass  # undo is best-effort — never let it block the actual move
...
def rename_file(path: str, name: str = "", new_name: str = "") -> str:
    try:
        ...
        try:
            from pragoncore.undo import push_undo
            final_path, orig_name = new_path, old_name
            push_undo(
                f"renamed {old_name} to {new_name}",
                lambda: final_path.rename(final_path.parent / orig_name),
            )
        except Exception:
            pass
...
def organize_desktop() -> str:
    ...
    try:
        for item in desktop.iterdir():
            # Don't touch folders, hidden files, or organized folders
