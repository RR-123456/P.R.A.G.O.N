...
def _get_api_key() -> str:
    path = _get_base_dir() / "api" / "api_keys.json"
...
_DANGEROUS_ACTIONS = {"restart", "shutdown", "toggle_wifi"}
...
def computer_settings(
    ...
    if action in _DANGEROUS_ACTIONS:
        # The confirmation token comes from the UI now, never from a tool
        # parameter the model itself can set — see pragoncore/confirm_gate.py.
        func = ACTION_MAP.get(action)
        if not func:
            return f"Unknown action: '{raw_action}'."
        if confirm_gate is None:
            # No gate wired in (e.g. an older caller) — fail safe by refusing
            # rather than silently skipping confirmation.
            return f"This would {action} the computer, but no confirmation gate is available. Not proceeding."
        verbs = {"restart": "restart", "shutdown": "shut down", "toggle_wifi": "toggle Wi-Fi on"}
        prompt = f"This will {verbs.get(action, action)} the computer."
        return confirm_gate.request(prompt, func)
