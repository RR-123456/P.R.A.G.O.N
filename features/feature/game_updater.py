...
from api import get_os, is_windows, is_mac, is_linux
...
def _click_first_profile_by_screenshot() -> bool:
    ...
    try:
        ...
        if not win:
            print("[GameUpdater] Steam window not found")
            return False
        ...
        if not colorful.any():
            print("[GameUpdater] Avatar color not found — clicking based on estimate")
        ...
        print(f"[GameUpdater] Missing library: {e}")
        return False
    except Exception as e:
        print(f"[GameUpdater] Profile detection failed: {e}")
        return False
...
def _handle_steam_profile_selection() -> bool:
    ...
    try:
        ...
        if not is_small and white_pixels <= 100:
            print("[GameUpdater] ℹ No profile dialog — Steam already logged in")
            return False
    except ImportError:
        pass
    except Exception:
...
def _handle_install_dialog(game_name: str) -> str:
    ...
    try:
        ...
        for _ in range(40):
            ...
            try:
                for hwnd in findwindows.find_windows(
                    title_re=r"(?i)(install|steam)", visible_only=True
                ):
                    try:
        ...
        print(f"[GameUpdater] pywinauto failed: {e}")
        return _handle_install_dialog_pyautogui(game_name, best_drive)
...
def _ensure_steam_running(steam_path: Path) -> bool:
    ...
    print("[GameUpdater] Steam could not be started")
    return False
...
def _search_steam_appid(game_name: str) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        print(f"[GameUpdater] AppID search failed: {e}")
