"""
pragoncore/wake_word.py — local wake-word detection ("Hey Jarvis") for P.R.A.G.O.N.

Ported and adapted from Mark-LIII's core/wake_word.py during the September 2026
upgrade pass. PRAGON previously had no local wake-word support at all — the mic
either streamed continuously or was muted by hand. This module lets the user
leave the mic muted and have PRAGON wake itself the moment it hears the trigger
phrase, entirely offline.

Design goals (unchanged from the source module):
...
CPU. The pretrained wake phrase used here is "Hey Jarvis" (openwakeword does not
ship a "Hey PRAGON" model out of the box; training a custom one is possible but
...
def install_and_download(logger: Callable[[str], None] = print) -> tuple[bool, str]:
    """
    One-click setup for the UI toggle: pip-install openwakeword if missing, then
